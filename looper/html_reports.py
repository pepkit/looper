...
class HTMLReportBuilder():
    ...
    def create_object_parent_html(prj, objs):
        # Generates a page listing all the project objects with links
        # to individual object pages
        reports_dir = os.path.join(prj.metadata.output_dir,
                                   "reports")
        ...
        with open(object_parent_path, 'w') as html_file:
            ...
            html_file.write(HTMLReportBuilder.create_navbar(prj, objs, reports_dir))
    ...
    def create_sample_parent_html(prj, objs):
    ...
    def create_object_html(prj, objs, nb, type, filename, index_html):
        # Generates a page for an individual object type with all of its
        # plots from each sample
        reports_dir = os.path.join(prj.metadata.output_dir, "reports")
        ...
        with open(object_path, 'w') as html_file:
            html_file.write(HTML_HEAD_OPEN)
            html_file.write(HTMLReportBuilder.create_navbar(prj, nb, reports_dir))
            html_file.write(HTML_HEAD_CLOSE)
    ...
    def create_status_html(prj, all_samples):
        ...
        with open(status_html_path, 'w') as html_file:
            html_file.write(HTML_HEAD_OPEN)
            html_file.write(HTMLReportBuilder.create_navbar(prj, all_samples, reports_dir))
            html_file.write(HTML_HEAD_CLOSE)
            ...
            for sample in prj.samples:
                ...
                log_file = os.path.join(prj.metadata.results_subdir,
    ...
    def create_sample_html(prj, all_samples, sample_name, sample_stats,
                           index_html):
        ...
        sample_page_relpath = os.path.relpath(
            html_page, prj.metadata.output_dir)
        single_sample = all_samples[all_samples['sample_name'] == sample_name]
    ...
    def create_project_objects(prj):
        # If a protocol produces project level summaries add those as
        # additional figures/links
        all_protocols = [sample.protocol for sample in prj.samples]
        ...
        for protocol in set(all_protocols):
            ...
            obj_links = []
            warnings = []
            ifaces = prj.interfaces_by_protocol[alpha_cased(protocol)]
            ...
            for iface in ifaces:
                ...
                for result in summary_results:
                    ...
                    result_img = str(result['thumbnail_path']).replace(
                                    '{name}', str(prj.name))
                    search = os.path.join(prj.metadata.output_dir,
                                          '{}'.format(result_file))
    ...
    def create_index_html(prj, objs, stats):
        ...
        objs_html_file.write("\t\t</style>\n")
        objs_html_file.write(HTML_TITLE.format(project_name=prj.name))
        navbar = HTMLReportBuilder.create_navbar(prj, objs, prj.metadata.output_dir)
        objs_html_file.write(navbar)
        objs_html_file.write(HTML_HEAD_CLOSE)

        # Add stats_summary.tsv button link
        tsv_outfile_path = os.path.join(prj.metadata.output_dir,
                                        prj.name)
            ...
            for row in stats:
                ...
                for value in table_row:                 
                    if value == sample_name:
                        ...
                        sample_page = HTMLReportBuilder.create_sample_html(
                                        prj, objs, sample_name,
        ...
        HTMLReportBuilder.create_sample_parent_html(prj, objs)

        # Create objects pages
        for key in objs['key'].drop_duplicates().sort_values():
            objects = objs[objs['key'] == key]
            object_filename = str(key) + ".html"
            HTMLReportBuilder.create_object_html(
                prj, objects, objs, key, object_filename, objs_html_path)

        # Create parent objects page with links to each object type
        HTMLReportBuilder.create_object_parent_html(prj, objs)

        # Create status page with each sample's status listed
        HTMLReportBuilder.create_status_html(prj, objs)

        # Add project level objects
        prj_objs = HTMLReportBuilder.create_project_objects(prj)
