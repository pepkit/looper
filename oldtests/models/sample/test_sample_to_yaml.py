...
OUTPUT_SPECS = [
    dict(spec) for spec in
    itertools.product(*[[(pk, oc) for oc in OUTPUT_COMBOS] for pk in PIPEKEYS])]
...
def prj(tmpdir, request):
    ...
    lines = [l.replace("{}: test".format(OUTDIR_KEY),
                       "{}: {}".format(OUTDIR_KEY, testpath))
              .replace("{}: pipelines".format(PIPELINE_INTERFACES_KEY),
                       "{}: {}".format(PIPELINE_INTERFACES_KEY, plif_file))
             for l in PROJECT_CONFIG_LINES if SAMPLE_SUBANNOTATIONS_KEY not in l]
...
def test_sample_yaml_outputs_inclusion(prj, outs_by_pipe):
    ...
    if multi_pipe_samples:
        raise Exception("Samples with non-1 number of pipeline keys: {}".
                        format(multi_pipe_samples))
    ...
    if missing:
        ...
        print("Submission folder contents: {}".format(os.listdir(prj.submission_folder)))
