...
def pytest_generate_tests(metafunc):
    ...
    try:
        ...
        _LOGGER.debug(
            "No indirect parameterization for test class: '{}'".format(metafunc.cls)
        )
    else:
...
def basic_pipe_iface_data(request):
    ...
    extension = (
        request.getfixturevalue("extension")
        if "extension" in request.fixturenames
        else ".py"
    )
    return {pipe_name + extension: {"name": pipe_name} for pipe_name in PIPELINE_NAMES}
...
def pi_with_resources(request, bundled_piface, resources):
    ...
    if "use_new_file_size" in request.fixturenames:
        file_size_name = (
            "min_file_size"
            if request.getfixturevalue("use_new_file_size")
            else "file_size"
        )
...
@pytest.mark.parametrize("exclude", powerset(PipelineInterface.REQUIRED_SECTIONS))
def test_requires_pipelines_and_protocol_mapping(
    ...
    missing = [
        s for s in PipelineInterface.REQUIRED_SECTIONS if s not in pipe_iface_config
    ]
    assert [] == missing, "Missing PI config section(s): {}".format(", ".join(missing))
    pipe_iface_config = {k: v for k, v in pipe_iface_config.items() if k not in exclude}
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ("choose_resource_package", {"file_size": 4}),
        (
            "get_arg_string",
            {"sample": Sample({"sample_name": "arbitrary-sample-name"})},
        ),
        ("get_attribute", {"attribute_key": "irrelevant-attr-name"}),
        ("get_pipeline_name", {}),
    ],
)
...
class PipelineInterfaceNameResolutionTests:
    ...
    def test_get_pipeline_name_explicit(self, name_and_ext_pairs):
        ...
        pi_conf_data = {
            pipeline: {"name": name} for pipeline, name in zip(pipelines, names)
        }
        pi = PipelineInterface(
            {PROTOMAP_KEY: {"ATAC": "ATACSeq.py"}, PL_KEY: pi_conf_data}
        )
...
class PipelineInterfaceResourcePackageTests:
    ...
    def test_negative_file_size_request(self, use_new_file_size, pi_with_resources):
        """Negative file size is prohibited."""
        pi = pi_with_resources
        for pipeline_name in pi.pipeline_names:
            negative_file_size = -10 * random.random()
            with pytest.raises(ValueError):
                pi.choose_resource_package(pipeline_name, file_size=negative_file_size)
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[(0, "default"), (4, "default"), (16, "midsize"), (64, "huge")],
    )
    ...
    def test_negative_file_size_prohibited(self, use_new_file_size, pi_with_resources):
        ...
        for pipe_name in pi_with_resources.pipeline_names:
            ...
            with pytest.raises(ValueError):
                pi_with_resources.choose_resource_package(pipe_name, file_size_request)
    ...
    def test_file_size_spec_not_required_for_default(
        ...
        resources_data = dict(
            zip(
                ...
                [
                    copy.deepcopy(data)
                    for data in [default_resources, midsize_resources, huge_resources]
                ],
            )
        )
        ...
        for pipe_name, pipe_data in pi.iterpipes():
            default_resource_package = pipe_data[RESOURCES_KEY][
                DEFAULT_COMPUTE_RESOURCES_NAME
            ]
            clear_file_size(default_resource_package)
            assert default_resource_package == pi.choose_resource_package(
                pipe_name, 0.001
            )

    @pytest.mark.parametrize(argnames="min_file_size", argvalues=[-1, 1])
    def test_default_package_new_name_zero_size(
        ...
        for pipe_name, pipe_data in pi_with_resources.iterpipes():
            ...
            observed_resource_package = pi_with_resources.choose_resource_package(
                pipe_name, 0
            )
    ...
    def test_file_size_spec_required_for_non_default_packages(
        self, use_new_file_size, bundled_piface, default_resources, huge_resources
    ):
        ...
        if use_new_file_size:
            resource_package_data["default"]["min_file_size"] = resource_package_data[
                "default"
            ].pop("file_size")
...
class ConstructorPathParsingTests:
    ...
    RELATIVE_PATH_DATA = [
        ("./arbitrary-test-pipelines", {}, "./arbitrary-test-pipelines"),
        (
    ...
    ABSPATH_ENVVARS = {
        "TEMP_TEST_HOME": "tmptest-home-folder",
        "TEMPORARY_SUBFOLDER": "temp-subfolder",
    }
    EXPECTED_PATHS_ABSOLUTE = [
        os.path.join(os.path.expanduser("~"), "code_home", "bioinformatics"),
    ...
    def bundled_piface(self, pipe_iface_data):
        return {PROTOMAP_KEY: {"ATAC": "ATACSeq.py"}, PL_KEY: pipe_iface_data}
    ...
    def test_no_path(self, config_bundles, piface_config_bundles, bundled_piface):
    ...
    @pytest.mark.parametrize(
        argnames=["pipe_path", "envvars", "expected"], argvalues=RELATIVE_PATH_DATA
    )
    def test_relative_path(
        self,
        config_bundles,
        piface_config_bundles,
        bundled_piface,
        pipe_path,
        envvars,
        expected,
...
class PipelinePathResolutionTests:
    ...
    def test_no_path(self, atacseq_piface_data, path_config_file, atac_pipe_name):
        ...
        _, full_pipe_path, _ = piface.finalize_pipeline_key_and_paths(atac_pipe_name)
    ...
    def test_non_dot_relpath_becomes_absolute(
        self, atacseq_piface_data, path_config_file, tmpdir, pipe_path, atac_pipe_name
    ):
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            (
                os.path.join("$HOME", "code-base-home", "biopipes"),
                os.path.join(os.path.expandvars("$HOME"), "code-base-home", "biopipes"),
            ),
            (
                os.path.join("~", "bioinformatics-pipelines"),
                os.path.join(os.path.expanduser("~"), "bioinformatics-pipelines"),
            ),
        ],
    )
    def test_absolute_path(
        ...
        exp_path = os.path.join(tmpdir.strpath, expected_path_base, atac_pipe_name)
        piface = PipelineInterface(path_config_file)
        _, obs_path, _ = piface.finalize_pipeline_key_and_paths(atac_pipe_name)
        assert exp_path == obs_path
