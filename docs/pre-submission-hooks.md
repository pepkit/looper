...
Write a python function that takes as input a `namespaces` object, which is a Python [`dict`](https://docs.python.org/3/tutorial/datastructures.html#dictionaries) of [looper variable namespaces](variable-namespaces.md). For their composition, please refer to [looper variable namespaces](variable-namespaces.md) section). This means that your pre-submission python function has access to *all of the metadata variables looper has access to to construct the primary command template*.
